import { formatTime, removeDashes } from "../../helpers/formatHelper"

describe("formatHelper", () => {

  test("test removeDashes", () => {
    const resultTest = removeDashes("Lille-Flandres"); 
    expect(resultTest).toBe("Lille Flandres");
  })
...
describe("Test de la fonction formatDatetime()", () => {
  ...
  test("erreur lever si format incorrect", () => {
    expect(() => {formatTime("c'est lundi")}).toThrow(); 
  })
});
})
